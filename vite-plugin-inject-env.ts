...
export function injectEnvPlugin(mode: 'development' | 'production'): Plugin {
    const generateEnvConfig = (mode: 'development' | 'production') => {
...
`;
        ...
        if (mode === 'development') {
            ...
            const publicPath = resolve(process.cwd(), 'env-config.js');
            writeFileSync(publicPath, envConfigContent);
        }
    ...
    return {
        name: 'inject-env',
        buildStart() {
            // Generate env-config.js for development too
            if (mode === 'development') {
                generateEnvConfig(mode);
            }
        },
        generateBundle() {
            const envFile = resolve(process.cwd(), `.env.${mode}`);
